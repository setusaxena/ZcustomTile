	...
	sap.ui.controller("view.CustomTile", {
		...
		onInit: function() {
			...
			oModel = new JSONModel({
				...
				nav: {
					navigation_target_url: (oTileApi.configurationUi && oTileApi.configurationUi.isEnabled() ? "" : this.navigationTargetUrl)
				},
				search: {
		...
		onPress: function(oEvent) {
			...
			if (oEvent.getSource().getScope && oEvent.getSource().getScope() === GenericTileScope.Display) {
				if (oTileApi.configurationUi.isEnabled()) {
					...
					if (sTargetUrl[0] === "#") {
						if (window.oDialog) {
							window.oDialog.destroy();
						}
						window.oDialog = sap.ui.xmlfragment(oTileConfig.navigation_semantic_action, "view.fragment.Popup", this);
						jQuery.sap.syncStyleClass("sapUiSizeCompact", this.getView(), window.oDialog);
						window.oDialog.open();
						this.setPopUp();
					}

					/* 
					Commented the standard code - Start
					if (sTargetUrl[0] === "#") {
					...
					Commented the standard code - End
					*/
				}
			}
		},
		...
		setPopUp: function() {
			...
			if (sap.ui.getCore().getConfiguration().getLanguage() === 'AR' || sap.ui.getCore().getConfiguration().getLanguage() === 'ar') {
				langu = 'A';
			} else {
				langu = 'E';
			}
			if (sap.ui.getCore().byId(oConfig.navigation_semantic_action + '--' + 'selFioriTiles') !== undefined) {

				var PopUp = sap.ui.getCore().byId(oConfig.navigation_semantic_action + '--' + 'selFioriTiles');
				var i18nModel = this.getView().getModel("i18n");
				PopUp.setModel(i18nModel, "i18n");
				PopUp.setTitle(oConfig.display_title_text);

				//document.getElementById(oConfig.navigation_semantic_action + '--' + "selFioriTiles-header").style.backgroundColor = "blue";
				//document.getElementById(oConfig.navigation_semantic_action + '--' + "selFioriTiles-footer").style.backgroundColor = "blue";
				//document.getElementById(oConfig.navigation_semantic_action + '--' + "selFioriTiles-title").style.color = "white";
				//document.getElementById(oConfig.navigation_semantic_action + '--' + "closePopup-BDI-content").style.color = "white";

				if (langu === 'A') {
					//PopUp._getCancelButton().setText("أغلق");
				} else {
					//PopUp._getCancelButton().setText("Close");
				}
				...
				oBinding.filter([oFilter, oFilter2, oFilter3, oFilter4, oFilter5]);
			}
		},
		...
		extractData: function(oData) {
			var name,
				aKeys = ["results", "icon", "title", "number", "numberUnit", "info", "infoState", "infoStatus", "targetParams", "subtitle",
					"stateArrow", "numberState", "numberDigits", "numberFactor"
				];
		...
		successHandlerFn: function(sRequestUrl, oResult) {
			...
			if (iRefreshInterval > 0) {
				...
				jQuery.sap.log.info(
					"Wait " + iRefreshInterval + " seconds before calling " + oConfig.service_url + " again",
					null,
		...
		loadData: function(oResult) {
			...
			if (sSAPLogonLanguage && sUrlWithUserDefaults.indexOf("sap-language=") === -1) {
				sUrlWithUserDefaults = sUrlWithUserDefaults + (sUrlWithUserDefaults.indexOf("?") >= 0 ? "&" : "?") + "sap-language=" +
					sSAPLogonLanguage;
			}
